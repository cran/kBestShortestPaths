...
Graph sidetrackGraph(Graph sourceG, Graph subtractG, double *dist);
...
List<List<GraphArc *> > *bestPaths(Graph graph, int source, int dest, int k)
{
	...
	double *dist = new double[nStates];
			...
			if ( pathGraph[source] ) {
				...
				while ( heapSize(pathQueue, endQueue) && --k ) {
					...
					*endRetired = pathQueue[0];
					...
					double lastWeight = newPath.last->weight;
					if ( lastHeapPos == -1 ) {
	...
	for (unsigned int i = 0; i < bad_created.size(); i++)
		delete[] bad_created[i];
...
void buildSidetracksHeap(int state, int pred)
{
	...
	if ( s ) {
		...
		if ( heapSize ) {
			...
			bad_created.push_back(heapStart);
